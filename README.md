...
一款可离线运行的英语单词记忆应用：内置专业版词典、艾宾浩斯遗忘曲线复习算法、近义词/词根群组挑战、AI 短文陪练、多阶段测验（英译汉 → 选单词 → 默写）、数据统计与备份恢复。核心学习功能**完全离线可用**；单词发音、AI 功能、从网络同步（WebDAV）为可选联网增强。

## 下载与当前版本

- **最新版**：**v2.1.3+21**（2026-09-05 发行，新增「从网络同步」：WebDAV/坚果云手动上传/下载备份快照；产品定性更正为「可离线运行的记单词 App」，隐私政策同步更新）
- **安装包**：`wordmem-v2.1.3-release.apk`（正式签名，66.8MB，GitHub Releases 分发；本地归档于 `2.0发行准备/`）
...
| 从网络同步 | 通过 WebDAV（支持坚果云）手动上传/下载备份快照：水位 + parent 链判定新旧、防呆确认、云端保留最近 10 份本机快照，换设备「先上传 → 再下载」接力 |
